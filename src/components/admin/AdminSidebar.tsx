...
const AdminSidebar: React.FC = () => {
  ...
  /* ===========================
      FIX 2: Required argument
     =========================== */
  const handleLogout = async () => {
    try {
      // Pass 'false' for the allDevices parameter
      await dispatch(logoutUser(false)).unwrap();
      setIsOpen(false);
      navigate("/admin/login");
    } catch (error) {
      // Local state is cleared by the slice matcher regardless of server response
      setIsOpen(false);
      navigate("/admin/login");
    }
  };
  ...
  return (
    <>
        ...
        <div className="flex items-center gap-1">
          ...
          <button onClick={handleBellClick} className="p-2 text-slate-400">
            {pushLoading ? (
              ...
              <BellOff size={20} className="text-red-300" />
            )}
          </button>
      ...
      {/* 🏛️ SIDEBAR */}
      <aside
        className={`fixed lg:sticky top-0 lg:top-[73px] left-0 z-[95] w-72 lg:w-64 bg-white h-full lg:h-[calc(100vh-73px)] border-r border-slate-200 transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        <div className="flex-1 overflow-y-auto p-6 pt-24 lg:pt-6">
          ...
          <nav className="space-y-1.5">
            ...
            >
              <PinIcon size={18} /> Conference Information
            </NavLink>
            <NavLink
              to="/admin/documents"
              onClick={() => setIsOpen(false)}
