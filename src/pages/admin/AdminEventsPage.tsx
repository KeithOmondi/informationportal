...
const AdminEventsPage = () => {
  ...
  const openModal = (event?: IEvent) => {
    ...
    if (event) {
      ...
      setForm({ title: "", description: "", location: "", startDate: "", endDate: "", status: "SCHEDULED", isMandatory: "false" });
    }
    setShowModal(true);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        <div>
          <h1 className="text-[#355E3B] font-serif text-2xl font-black">Events</h1>
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          <div className="relative flex-1 md:w-48">
            ...
            <input 
              ...
              className="w-full bg-slate-50 border-none rounded-lg pl-9 pr-3 py-2 text-xs focus:ring-1 focus:ring-[#355E3B]/20 outline-none"
          ...
          <button
            onClick={() => openModal()}
            className="flex items-center gap-2 bg-[#355E3B] text-white px-4 py-2 rounded-lg font-black text-[10px] uppercase tracking-widest hover:bg-[#2a4b2f] transition-all shadow-md active:scale-95"
          >
      ...
      <div className="grid grid-cols-1 gap-3">
        {loading ? (
          <div className="py-10 flex justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#355E3B]" /></div>
        ) : filteredEvents.length === 0 ? (
          <div className="py-12 text-center bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
            <p className="text-slate-400 font-serif italic text-sm">No records found.</p>
          </div>
        ) : (
          filteredEvents.map((event) => (
            <div key={event._id} className="bg-white border border-slate-100 rounded-xl hover:border-[#355E3B]/20 transition-all flex flex-col sm:flex-row overflow-hidden group shadow-sm">
              ...
              <div className="p-4 flex-1 flex flex-col justify-center min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <h3 className="text-[#355E3B] text-sm font-bold truncate flex-1">{event.title}</h3>
                </div>

                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-slate-500">
                  <span className="flex items-center gap-1 text-[9px] font-bold uppercase"><MapPin size={12} className="text-[#C5A059]" /> {event.location}</span>
                  <span className="flex items-center gap-1 text-[9px] font-bold uppercase"><Timer size={12} className="text-[#355E3B]" /> {getTimeRemaining(event.startDate)}</span>
                </div>
              </div>

              <div className="flex sm:flex-col border-t sm:border-t-0 sm:border-l border-slate-50">
                <button onClick={() => openModal(event)} className="flex-1 p-3 text-slate-400 hover:text-[#C5A059] hover:bg-slate-50 transition-colors flex justify-center border-r sm:border-r-0 sm:border-b border-slate-50"><Edit2 size={16} /></button>
      ...
      {/* MODAL - Fixed Description and Z-Index */}
      {showModal && (
        <div className="fixed inset-0 bg-[#355E3B]/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-100">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white">
              <h2 className="text-[#355E3B] font-serif text-xl font-black">{editingEvent ? "Edit Entry" : "New Entry"}</h2>
            ...
            <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              ...
              <div 
                ...
                className="relative aspect-[21/9] w-full rounded-xl bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden cursor-pointer hover:border-[#355E3B]/30 transition-colors"
              >
                {previewUrl ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="col-span-2">
                  ...
                  <input type="text" name="title" value={form.title} onChange={handleChange} className="w-full bg-slate-50 rounded-lg px-3 py-2 text-xs outline-none focus:ring-1 focus:ring-[#C5A059]/30" />
                </div>

                {/* DESCRIPTION - Re-added Fix */}
                <div className="col-span-2">
                  ...
                  <div className="relative">
                    <textarea 
                      ...
                      rows={2} 
                      className="w-full bg-slate-50 rounded-lg pl-3 pr-3 py-2 text-xs outline-none focus:ring-1 focus:ring-[#C5A059]/30 resize-none"
                ...
                <div className="col-span-2">
                  ...
                  <input type="text" name="location" value={form.location} onChange={handleChange} className="w-full bg-slate-50 rounded-lg px-3 py-2 text-xs outline-none focus:ring-1 focus:ring-[#C5A059]/30" />
                </div>

                {/* DATES */}
                <div>
                  <label className="text-[8px] font-black uppercase tracking-widest text-slate-400 mb-1 block">Start</label>
                ...
                <div>
                  <label className="text-[8px] font-black uppercase tracking-widest text-slate-400 mb-1 block">End</label>
            ...
            <div className="p-4 bg-slate-50 flex gap-2 justify-end">
              <button onClick={() => setShowModal(false)} className="px-4 py-2 text-slate-400 text-[10px] font-black uppercase hover:bg-slate-200 rounded-lg transition-all">Cancel</button>
              <button onClick={handleSubmit} disabled={loading} className="px-6 py-2 bg-[#355E3B] text-white rounded-lg text-[10px] font-black uppercase tracking-widest shadow-md disabled:opacity-50">
                {editingEvent ? "Commit" : "Publish"}
              </button>
            </div>
          </div>
        </div>
