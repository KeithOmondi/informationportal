...
const DrEvents = () => {
  ...
  // Fix: Use a stable filter check and AbortController to prevent race conditions/loops
  useEffect(() => {
    ...
    return () => {
      // Clean up the request if the component unmounts or filter changes
      promise.abort();
    };
  }, [dispatch, filter]);
    ...
    <div className="max-w-7xl mx-auto space-y-8 p-4 md:p-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      {loading && events.length === 0 ? (
        <div className="py-32 flex flex-col items-center gap-4">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-300">Synchronizing Registry...</span>
        </div>
      ) : error ? (
        ...
        <div className="py-24 flex flex-col items-center justify-center bg-slate-50/50 rounded-[3rem] border-2 border-dashed border-slate-100 text-slate-300">
           ...
           <p className="mt-4 font-serif italic text-sm text-slate-400">No active records found for the selected filter.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
          {events.map((event: IEvent) => {
            ...
            return (
              ...
              >
                {/* THUMBNAIL */}
                <div className="w-full sm:w-52 h-52 rounded-[2rem] overflow-hidden bg-slate-100 relative shrink-0">
                  {event.image?.url ? (
                    <img 
                      src={event.image.url} 
                ...
                <div className="flex-1 py-4 pr-6 flex flex-col justify-between">
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{formatDate(event.startDate)}</span>
                      </div>
                      {event.isMandatory && (
                        <span className="bg-red-50 text-red-600 text-[8px] font-black uppercase px-2.5 py-1 rounded-lg border border-red-100">Mandatory</span>
                      )}
                    </div>

                    <h3 className="text-[#1a3a32] font-serif text-xl md:text-2xl font-black group-hover:text-[#c2a336] transition-colors leading-tight line-clamp-2">
                      {event.title}
                    </h3>
                    
                    <div className="flex items-center gap-2 text-slate-500">
                      <MapPin size={14} className="text-[#c2a336]" />
                      <span className="text-[10px] font-bold uppercase tracking-tight">{event.location || 'Registry Chambers'}</span>
                    </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-slate-50 flex items-center justify-between">
                    {timeLeft ? (
                      <div className="flex items-center gap-2 text-[#1a3a32] bg-slate-50 px-3 py-1.5 rounded-full">
                        <Timer size={12} className="text-[#c2a336]" />
                        <span className="text-[9px] font-black uppercase tracking-tighter">{timeLeft}</span>
                      </div>
                    ) : (
                      <span className="text-[9px] font-black uppercase text-slate-300 tracking-[0.2em]">Live Session</span>
                    )}
                    
                    <button className="flex items-center gap-2 text-[10px] font-black uppercase text-[#1a3a32] hover:text-[#c2a336] transition-colors group/btn">
